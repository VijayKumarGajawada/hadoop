...
public static class MyMapper extends Mapper<LongWritable,Text,Text,Text>
{
	public void map(LongWritable key, Text value, Context contx) throws IOException, InterruptedException
	{
		...
		String  s_1 = stadium + "," + 1;
		contx.write(new Text(season), new Text(s_1));
	}
}
...
public static class MyReducer extends Reducer<Text,Text,Text,Text>
{
	public void reduce(Text key,Iterable<Text> values, Context context) throws IOException, InterruptedException
	{
		...
		for(Text val : values)
		{
			...
			String stadium = valArr[0];
			count = Integer.parseInt(valArr[1]);
			if(hmap.containsKey(stadium))
			{
				int currentCount = hmap.get(stadium);
				hmap.put(stadium, currentCount+count);
			}
			else
			{
				hmap.put(stadium, count);
			}
		}
		for(Map.Entry<String,Integer> entry : hmap.entrySet())
		{
			String stadium = entry.getKey();
			int stadiumCount = entry.getValue();
			tmap.put(stadiumCount,stadium);
		}
		String result = "Most matches held at " + tmap.get(tmap.lastKey()) + 
				"\n\tLeast matches held at " + tmap.get(tmap.firstKey());
